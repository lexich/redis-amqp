...
class Client {
        ...
        if (channel === channelMsg) {
          this.eventHandler(msg, (err, msg)=> {
            if (err) {
              ...
              LOG.log("fail    : " + this.id + ":" + msg);
            } else {
              LOG.log("received: " + this.id + ":" + msg);
            }
          });
   ...
   * @return {Promise}                      App flow promise
   */
  runPublisher(config, _incativeSubs) {
    ...
    if (!config.subscribers.length) {
      ...
      return new Promise((resolve)=> {
        ...
        return this.runPublisher(config, incativeSubs);
      });
    } else {
    ...
    return asyncNext(config.subscribers, 0, (id)=> {
      ...
      const msg = this.repeatMsg.length ? this.repeatMsg.pop() : this.getMessage();
    ...
    .then(()=> this.runPublisher(config, incativeSubs));
  }
  /**
